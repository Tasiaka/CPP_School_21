...
int SnakeFSM::NextDeadlineMs() const {
  return (state_ == SnakeState::FastRunning) ? model_->GetFastTickMs()
                                             : model_->GetTickMs();
}
...
bool SnakeFSM::ShouldTick() {
  ...
  if (now - last_ms_ >= static_cast<uint64_t>(need)) {
    last_ms_ = now;
    return true;
  }
  return false;
...
void SnakeFSM::Step() {
  switch (state_) {
    ...
    case SnakeState::Pause:
      ...
      break;
    case SnakeState::Win:
      HandleGameOverOrWin(true);
      break;
    case SnakeState::GameOver:
      HandleGameOverOrWin(false);
      break;
...
void SnakeFSM::HandleRunning() {
  if (new_input_) {
    if (last_input_ == Pause) {
      model_->SetPaused(true);
      state_ = SnakeState::Pause;
      return;
    }
    if (last_input_ == Terminate) {
      state_ = SnakeState::Exit;
      return;
    }
    if (last_input_ == Left) model_->TurnLeft();
    if (last_input_ == Right) model_->TurnRight();
    if (last_input_ == Action && hold_) {
      model_->SetAccelerate(true);
      state_ = SnakeState::FastRunning;
    }
  }
  if (ShouldTick()) model_->Tick();
  if (model_->IsWin()) {
    state_ = SnakeState::Win;
    model_->SetPaused(false);
    return;
  }
